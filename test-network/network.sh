...
export PATH=${ROOTDIR}/tool/bin:${PWD}/tool/bin:$PATH
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating company Org Identities"
    ...
    infoln "Creating person Org Identities"
    ...
    infoln "Creating government Org Identities"
    ...
    infoln "Creating targetc Org Identities"
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    infoln "Creating TagrgetC Org Identities"

    createTargetC

    infoln "Creating TagrgetC Orderer Org Identities"

    createTargetCOrderer

  fi

  infoln "Generating CCP files for Company \ Person \ Government \ TargetC"
...
## Call the script to deploy a chaincode to the channel
function invokeCC() {
  scripts/invokeCC.sh $CHANNEL_NAME $CC_NAME  $CC_INOVKE_FCN $CLI_DELAY $MAX_RETRY 

  if [ $? -ne 0 ]; then
    fatalln "invoke chaincode failed"
  fi
}
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm compose_orderer.example.com compose_peer0.company.example.com compose_peer0.person.example.com compose_peer0.government.example.com compose_peer0.targetc.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/company/msp organizations/fabric-ca/company/tls-cert.pem organizations/fabric-ca/company/ca-cert.pem organizations/fabric-ca/company/IssuerPublicKey organizations/fabric-ca/company/IssuerRevocationPublicKey organizations/fabric-ca/company/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/person/msp organizations/fabric-ca/person/tls-cert.pem organizations/fabric-ca/person/ca-cert.pem organizations/fabric-ca/person/IssuerPublicKey organizations/fabric-ca/person/IssuerRevocationPublicKey organizations/fabric-ca/person/fabric-ca-server.db'
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/targetc/msp organizations/fabric-ca/targetc/tls-cert.pem organizations/fabric-ca/targetc/ca-cert.pem organizations/fabric-ca/targetc/IssuerPublicKey organizations/fabric-ca/targetc/IssuerRevocationPublicKey organizations/fabric-ca/targetc/fabric-ca-server.db'
...
if [ "$MODE" == "up" ]; then
  ...
  infoln "invoke function on channel '${CHANNEL_NAME}'"
  invokeCC
else
  printHelp
  exit 1
